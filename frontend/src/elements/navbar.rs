...
pub fn NewPostPopup(cx: Scope, hide: UseState<bool>) -> Element {
   ...
   const BUTTON_CLASS: &str = "grid grid-cols-[20px_1fr] gap-4 pl-4 justify-center items-center w-full h-12 border-ynavbar-border-color";
...
pub fn NavButton<'a>(cx: Scope<'a, NavButtonProps<'a>>) -> Element {
    ...
    let selected_bgcolor = maybe_class!("bg-slate-500",
        matches!(cx.props.highlight, Some(true))
    );
    cx.render(rsx! {
        button {
