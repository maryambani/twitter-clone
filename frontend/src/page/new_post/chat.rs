...
use crate::{prelude::*, fetch_json, util::api_client::{self, ApiClient}};
...
pub fn NewChat(cx: Scope) -> Element {
    ...
    let form_onsubmit = async_handler!(&cx, [api_client, page_state, toaster, router], move |_| async move {
            use uchat_domain::post::{Headline, Message};
            ...
            match response {
                ...
                Err(e) => {
                    toaster.write().error(format!("Post failer: {e}"), Duration::seconds(3));
                },
            }
        }
    );
