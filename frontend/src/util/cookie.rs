...
pub fn remove_session() {
    ...
    document()
        .set_cookie(&cookie).unwrap()
}

pub fn set_session(signature: String, id: SessionId, expires: DateTime<Utc>) {
    let cookie = format_cookie(format_kv(uchat_cookie::SESSION_ID, id.to_string()),
    expires);

    document().set_cookie(&cookie).unwrap();

    let cookie = format_cookie(
        format_kv(uchat_cookie::SESSION_SIGNATURE, signature), expires);
